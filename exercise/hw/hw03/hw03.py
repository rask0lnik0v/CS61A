...
def totals_tree(m):
    ...
    return tree(label, [left_branch, right_branch])# 记忆怎么用adt来构建树
...
def replace_leaf(t, find_value, replace_value):
    ...
    newlabel = label(t)
    if is_leaf(t):
        if newlabel == find_value:
            newlabel = replace_value
        return tree(newlabel)
    else:
        return tree(newlabel, [replace_leaf(branch, find_value, replace_value) for branch in branches(t)])
...
def has_path(t, word):
    ...
    def path_helper(t, word, tries):
        ...
        if tries == word:
            ...
            for b in branches(t):
                if path_helper(b, word, tries):
                    return True
        return False

    return path_helper(t, word, "")
...
def quadratic(x, a, b, c):
    ...
    if t > lb and t < ub:
        if a > 0:
            low = a*pow(t, 2) + b*t + c
            high = max((a*pow(lb, 2) + b*lb +c), (a*pow(ub, 2) + b*ub +c))
        else:
            high = a*pow(t, 2) + b*t + c
            low = min((a*pow(lb, 2) + b*lb +c), (a*pow(ub, 2) + b*ub +c))
    else:
        low = min((a*pow(lb, 2) + b*lb +c), (a*pow(ub, 2) + b*ub +c))
        high = max((a*pow(lb, 2) + b*lb +c), (a*pow(ub, 2) + b*ub +c))
