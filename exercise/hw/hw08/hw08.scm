...
(define (exp? expr)
  ...
  (cond ((number? expr) #f)
	((variable? expr) #f)
        (else (and (equal? (car expr) '^) (variable? (first-operand expr)) (number? (second-operand expr))))
  )
...
(define (derive-exp expr var)
  'YOUR-CODE-HERE
  (cond ((not (same-variable? (first-operand expr) var)) 0)
	(else (make-product (second-operand expr)
