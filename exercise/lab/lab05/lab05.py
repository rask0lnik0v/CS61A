def couple(s, t):
    ...
    merge = s + t
    half = len(merge)//2
    return [[merge[k], merge[k + half]] for k in range(half)]    
...
def closer_city(lat, lon, city_a, city_b):
    ...
    if distance(city_tmp, city_a) > distance(city_tmp, city_b):
...
def sprout_leaves(t, leaves):
    ...
    '''
    参考北大学长的写法
    一方面注意本题需要返回一个新的树，另一方面注意列表推导式在树建立时的用法
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[len(deck)//2 + k//2] if k%2 else deck[k//2] for k in range(len(deck))]
...
def add_trees(t1, t2):
    ...
    while i < len(branches(t1)) and i < len(branches(t2)):
        ...
        result_branches = result_branches + [new_branches]
        i = i + 1
    ...
    "修改版，使用了zip来同时迭代两个序列，同时使用列表推导式进行branches的生成"
    result_label = label(t1) + label(t2)
    result_branches = [add_trees(b1, b2) 
                        for b1,b2 
                        in zip(branches(t1), branches(t2))]
    i = min(len(branches(t1)), len(branches(t2)))
    result_branches = result_branches + branches(t1)[i:]
    result_branches = result_branches + branches(t2)[i:]
    ...
    """
    递归的写法。核心思路还是构建helper函数。注意，最后对于剩下的多出来分支的处理很妙。但不建议第一次写就这么做。
    def build_result_branches(bs1, bs2):
        "bs1 and bs2 are the remaning branches."
        if not bs1:
            return bs2
        if not bs2:
            return bs1
    ...
    result_label = label(t1) + label(t2)
    result_branches = build_result_branches(
                        branches(t1),
                        branches(t2))
...
def construct_sent(word, table):
    ...
    while word not in ['.', '!', '?']:
        "*** YOUR CODE HERE ***"
        newword = str(random.choice(table[word]))
        result = result + word + " "
        word = newword
