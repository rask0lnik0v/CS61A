def insert_into_all(item, nested_list):
    ...
    [[0], [0, 1, 2], [0, 3]]
    """
    return [[item] + lis for lis in nested_list]
...
def subseqs(s):
    ...
    [[]]
    """
    if s == []: # 边界情况可以从空list中看出来
        return [s]
    else:
        not_in = subseqs(s[1:])
        return insert_into_all(s[0], not_in) + not_in
        # 每次选择是否将s[0]加入本次的序列中
...
class Keyboard:
    ...
    def typing(self, typing_input):
        ...
        string = ''
        for each in typing_input:
            string += self.press(each)
        return string
...
def make_advanced_counter_maker():
    ...
    """
    global_count = 0
    def counter(): # 这个是counter
        count = 0
        def process(string): # 这个是处理函数
