...
def num_splits(s, d):
    ...
    def helper(s, difference):
        ...
        # 通过加减判断取不取该值
    return helper(s, 0) // 2 # 除以2是因为一半重复
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            ...
            res = str(code_char) + res
            possibilities.append((res, cost))
        # Insert
        res, cost = helper_align(skeleton_idx, code_idx + 1)
        res = "+[" + str(code_char) + "]" + res
        possibilities.append((res, cost + 1))
        # Delete
        res, cost = helper_align(skeleton_idx + 1, code_idx)
        res = "-[" + str(skel_char) + "]" + res
        possibilities.append((res, cost + 1))
        return min(possibilities, key=lambda x: x[1])
    result, cost = helper_align(0, 0) # 通过这一行判断helper函数究竟要输出什么
    return result
...
def foldl2(link, fn, z):
    ...
    """
    def step(x, g): 
        # 然后思考step要做什么，step就是foldr中的fn，地一个参数是link.first，第二个参数是foldr的递归
        # 再思考从外部传入的z，z是foldr中最后参与递归的，也是foldl中最先参与迭代的
        # 那么如下的写法中，fn(g(y), x)中的g(y)相当于先用这种方式将z递归到最底层（因为每次都会先递归它），但是之后会先计算link中的最后一个与它的fn。但是无妨，看表达式会发现先计算0-3与先计算0-1是一样的
        "*** YOUR CODE HERE ***"
        def f(y):
            return fn(g(y), x)
        return f
    return foldr(link, step, identity)(z) # 首先思考这里为啥有一个参数z？对于foldl而言，其中z是到边界处返回的值，这里到边界处返回的是identity，一个返回自身的函数，也就是返回z本身。而在非边界情况下，返回fn，也就是step，说明step本身要返回带一个参数的函数
    # 本质上而言，foldl是尾递归，类似迭代
    # 而foldr是递归，这里就是要用递归实现迭代的效果
