...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while True:
            if place_now == beehive:
                break
            elif place_now.bees == []:
                place_now = place_now.entrance
                place_index += 1
            else:
                if self.min_range <= place_index <= self.max_range:
                    break
                else:
                    place_now = place_now.entrance
                    place_index += 1
        ...
        if place_now == beehive:
            return None
        return rANTdom_else_none(place_now.bees)
        # END Problem 3 and 4
    ...
    def reduce_armor(self, amount):
        ...
        def reflect(amount):
            ...
            for bee in self.place.bees:
                if bee.armor > amount:
                    bee_list.append(bee)
            for bee in self.place.bees[:]: # 需要对拷贝的蜜蜂序列进行攻击 
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.has_a_queen:
            self.is_real_queen = False
        else:
            self.is_real_queen = True

        QueenAnt.has_a_queen = True
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.is_real_queen:
            self.throw_at(self.nearest_bee(gamestate.beehive))
            place_now = self.place.exit
            while place_now != None:
                if place_now.ant:
                    if not place_now.ant.is_doubled:
                        place_now.ant.is_doubled = True
                        place_now.ant.damage *= 2
                place_now = place_now.exit
        else:
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        index = self.is_real_queen
        ScubaThrower.reduce_armor(self, amount)
        if amount >= 1 and index:
            bees_win()
        # END Problem EC
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if self.forward is False: # 通过forward判断方向
            if isinstance(self.place.entrance, Hive) :
                destination = self.place
            else:
                destination = self.place.entrance
                # print("DEBUG: it turns backward")
            self.forward = True
        # END EC
        if self.blocked():
...
def make_slow(action, bee):
    ...
    "*** YOUR CODE HERE ***"
    def new_action(gamestate): # 注意gamestate是全局的
        if gamestate.time % 2 == 0: # 注意要求是奇数不能动，不是自定义下一个回合不能动
            action(gamestate)
...
def make_scare(action, bee):
    ...
    def new_action(gamestate):
        # print("DEBUG: forward state:", bee.has_been_scared) debug用
        bee.forward = False # 这里只需要改forward即可
        # print("DEBUG: forward changed") debug用
        action(gamestate)
...
def apply_status(status, bee, length):
    ...
    new_action = status(bee.action, bee) # 思路是分为老action与新action，然后使用新函数判断语句对其进行选择
    def changed_action(gamestate):
        nonlocal length
        if length > 0:
            new_action(gamestate)
            length -= 1
            if bee.forward is False: # 注意，这一步对于forward的更新是必要的，因为会出现slow与scare叠加的情况，如果不如此，即便过了scared，仍然会有一次backward，因为此时forward还是False（没有动所以没有改过来）
                bee.forward = True
        else:
            old_action(gamestate)

    bee.action = changed_action # 替换成有选择的action
    # print("DEBUG:action was changed") debug用
    # END Problem Optional 4
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem Optional 4
        "*** YOUR CODE HERE ***"
        if target and not target.has_been_scared: # 如何保证bee只被scare一次，在这里进行判断，而不是在make_scare与apply甚至bee.action中进行判断
            apply_status(make_scare, target, 2)
            target.has_been_scared = True
            # print("DEBUG: bee was scared")
        # END Problem Optional 4
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        distance = 0
        insect_dic = {}
        while place_now != beehive:
            if place_now.ant is not self and place_now.ant is not None:
                insect_dic[place_now.ant] = distance
            for each in place_now.bees:
                if each is not None:
                    insect_dic[each] = distance
            distance += 1
            place_now = place_now.entrance
        return insect_dic
        # END Problem Optional 5
