...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def compare(score0, score1, choose=who, lastscore=last_score, highest=running_high):
        if choose:
            incre = score1 - lastscore
            if incre > highest:
                print('{} point(s)! The most yet for Player {}'.format(incre, choose))
            return announce_highest(choose, score1, max(incre, highest))
        else:
            incre = score0 - last_score
            if incre > running_high:
                print('{} point(s)! The most yet for Player {}'.format(incre, choose))
            return announce_highest(choose, score0, max(incre, highest))
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    simulation = make_averaged(roll_dice, trials_count)
    maxscore = -1
    maxindex = 0
    for i in range(1, 11):
        now = simulation(i, dice)
        if now > maxscore:
            maxscore = now
            maxindex = i
