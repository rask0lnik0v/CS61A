...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***" # 这里其实是对整个call expr做eval
        operator = scheme_eval(first, env) # 先对操作符进行eval，会直接通过lookup查找
        validate_procedure(operator) # 注意如果不是procedure会直接报错，所以这里直接调用即可，不需要写判断语句

        # 对于20题，还有一出要改的是这里，先用apply_macro翻译出语句，然后用正常的scheme_eval执行即可
        if isinstance(operator, MacroProcedure):
            return scheme_eval(operator.apply_macro(rest, env), env)

        operands = rest.map(lambda x: scheme_eval(x, env) ) # 使用lambda将多输入转为单输入，之前的hw也有类似题目
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        new_env = procedure.make_call_frame(args, env) # eval_all最后一个肯定要开尾递归
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        formal_now = formals
        val_now = vals
        while formal_now is not nil:
            child_frame.define(formal_now.first, val_now.first)
            formal_now = formal_now.rest
            val_now = val_now.rest
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        "*** YOUR CODE HERE ***"
        pair_now = args
        while pair_now is not nil:
            python_args.append(pair_now.first)
            pair_now = pair_now.rest
        if self.use_env:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        new_expr = Pair(formals, body)
        lambda_procedure = do_lambda_form(new_expr, env) 
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    return expressions.first # 返回first是因为其结构如下:Pair(real_expr, nil)，直接返回全部会多()
    # END PROBLEM 6
...
def do_and_form(expressions, env):
    ...
    while expr is not nil:
        if expr.rest is nil: # 对于and语句而言，只有最后一条可能是尾递归
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            if clause.rest is nil: # 注意此时clause才是第一个cond语句
                return test
            else:
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
    expr_now = bindings
    while expr_now is not nil:
        validate_form(expr_now.first, 2, 2)

        binding = expr_now.first
        name = binding.first
        value = scheme_eval(binding.rest.first, env) # 注意eval需要的是expr，因而需要再加一个first

        names = Pair(name, names)
        values = Pair(value, values)

        expr_now = expr_now.rest
...
class MuProcedure(Procedure):
    ...
    "*** YOUR CODE HERE ***"
    def make_call_frame(self, args, env):
        return env.make_child_frame(self.formals, args) # 这里与lambda的区别便是一个使用的env是init时的，一个使用的是call时的
    # END PROBLEM 18
...
def complete_apply(procedure, args, env):
    ...
    print("DEBUG: complete_apply", val)
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        if tail and not scheme_symbolp(expr) and not self_evaluating(expr):
            return Thunk(expr, env) # return thunk就是将中间结果直接存储在thunk中而不是创建递归调用的环境进行递归
